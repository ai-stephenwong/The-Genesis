...
- **Runtime version cross-check (mandatory):** Before writing the Dockerfile, cross-reference the runtime version from: (1) `.nvmrc` or equivalent version pinning file, (2) `package.json engines`, (3) `stack.md`. All three must agree. Any discrepancy is a blocking finding — halt and escalate to the orchestrator.
