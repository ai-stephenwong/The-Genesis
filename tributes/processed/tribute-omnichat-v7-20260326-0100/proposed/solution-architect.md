...
| 7 | Runtime version pinning | `.nvmrc` (Node.js) / `.python-version` / `.tool-versions` | Pins exact runtime version; consumed by developer, devops-engineer, and CI. Must match the version in `stack.md`. |
...
- **Runtime version pinning (mandatory):** Produce a version pinning file (`.nvmrc` for Node.js, `.python-version` for Python, `.tool-versions` for polyglot projects) that pins the exact runtime version selected in `stack.md`. This file is consumed by the developer (to verify their local environment), the devops-engineer (to set the Dockerfile base image and CI runner version), and CI (for setup-node/setup-python actions). The version in the pinning file must exactly match `stack.md`. Include this file in the sign-off checklist.
