...
- **Check cross-service env vars are documented, name-matched, and environment-appropriate** — for every `import.meta.env.VITE_*` or `process.env.NEXT_PUBLIC_*` reference in the frontend, and every `process.env.*` or `c.env.*` reference in the backend, verify: (1) the exact key name matches the **Environment Variable Contract** in `env-contract.md` (this is the single source of truth — any env var in code that does not appear in the contract, or uses a different key name, is an automatic **Critical** finding); (2) the exact name also appears in `agent_docs/project/deployment.md`; (2) no silent hardcoded fallback exists for a required env var (e.g. `VITE_API_BASE_URL || 'https://...'` is a Major finding — a missing required env var must fail loudly, not fall through to a wrong host); (3) **environment-context verification:** for any configuration value referencing an external service domain (API base URLs, CDN origins, CSP directives in `vercel.json` or `_headers`, OAuth redirect URIs, env var fallback values), verify the value is appropriate for the deployment environment the file governs — any hardcoded production domain appearing as an env var fallback value, in a CSP directive, or in a config file active in a non-production environment is an automatic **Critical** finding ("production URL in non-production context"); treat any env var read with a production-domain fallback (e.g. `?? 'https://api.example.com'`) as Critical regardless of the variable's name. Also verify CORS origins and API base URLs are documented with the correct deployed values.
...
- **Deploy method verification (mandatory):** In every CI/CD workflow, verify that deployments are triggered by git push / git merge — not by direct CLI commands (`vercel deploy`, `wrangler deploy`) or MCP deploy tools. The only sanctioned exception is `railway up` inside GitHub Actions (see devops-engineer Railway exception). Any workflow that runs `vercel deploy`, `vercel --prod`, `wrangler deploy`, or equivalent direct deploy commands is an automatic **Critical** finding — these commands bypass the CI/CD pipeline even when run inside GitHub Actions, because they upload local artifacts instead of deploying from the git-connected source.
