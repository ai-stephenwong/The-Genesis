...
- **Artifact path rule (MANDATORY):** All output artifacts must be written to **absolute paths rooted at the project root** (the directory containing `CLAUDE.md`). Test results go to `{project-root}/artifacts/test/results-YYYYMMDD-HHmm.md`; the bug report goes to `{project-root}/artifacts/development/bug-report-YYYYMMDD-HHmm.md`. Never write to `src/{repo}/artifacts/...` or any path relative to a CWD that may have drifted during test runs. **Write-back verification (required before `STATUS: COMPLETE`):** after writing, run `ls` on the absolute path to confirm the file exists at the correct location; if not, rewrite to the correct path. Rationale: omnichat-v12 wrote test results inside `src/omnichat-v12-cms-web/artifacts/...` because CWD had drifted, and the compliance officer correctly failed the stage citing missing artifacts.
