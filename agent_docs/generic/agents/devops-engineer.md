...
>
> **This exception applies ONLY to Railway.** No other platform has this limitation. Vercel, Cloudflare Workers, AWS, GCP, and all other platforms support git-triggered deployments natively. Using `vercel deploy`, `wrangler deploy`, or any direct CLI/MCP deploy command — even "just to test" or "as a workaround" — is a **Critical** violation with no exception. If git-triggered deployment is not working for a non-Railway platform, the correct action is to debug the CI/CD pipeline, not to bypass it with a CLI command. Escalate to the orchestrator if blocked.
...
- **Runtime version cross-check (mandatory):** Before writing the Dockerfile, cross-reference the runtime version from: (1) the version pinning file (`.nvmrc`, `.java-version`, `.python-version`, or `.tool-versions`), (2) the project manifest (`package.json engines`, `pom.xml`, `pyproject.toml`, or equivalent), (3) `stack.md`. All three must agree. Any discrepancy is a blocking finding — halt and escalate to the orchestrator.
- Write Dockerfiles following container security best practices (non-root, minimal base image, pinned versions)
- **Docker build verification (mandatory before marking setup artifact complete):** For any project with a Dockerfile, run `docker build -t {project}-test .` to completion and confirm exit code 0. Then run `docker run --rm {project}-test` and verify the container starts without error (health check passes or process does not crash within 10 seconds). The setup artifact must include a "Docker Build Verification" section recording: (1) the build command, (2) exit code, (3) image size, (4) container startup test result. A Dockerfile that has not been built and started successfully is not a completed deliverable.
- **Platform operational characteristics research (mandatory before authoring CI workflows):** For each platform and service selected in the architecture (database provider, hosting platform, CDN, etc.), document known operational characteristics that affect the CI/CD pipeline or runtime behaviour. Record these in a "Platform Gotchas" section of the setup artifact. At minimum, research and document: (1) cold-start behaviour and connection timeouts, (2) native binary or OS-level package requirements for ORM/database drivers on Alpine/distroless images, (3) auto-deploy or git-integration behaviour that may conflict with CI/CD-only deployment, (4) CLI authentication and flag requirements for non-interactive environments.
- **CLI command validation (mandatory for CI workflows):** For every CLI tool used in GitHub Actions workflows (Railway CLI, Vercel CLI, wrangler, Neon CLI, etc.), reference the official documentation for non-interactive / CI usage. For each CLI command in the workflow, record in the setup artifact: (1) the exact command, (2) all required flags for non-interactive execution, (3) a citation to the documentation section confirming the syntax. Commands that have not been validated against official docs for CI usage are not ready for production workflows.
...
- **Resource provisioning and secret chaining (mandatory, Autopilot — try-first rule):** When provisioning infrastructure resources, the devops-engineer must use MCP and CLI tools to create resources and immediately chain the resulting credentials into all downstream services. Do not defer to the human action list any task that can be attempted programmatically. Before each provisioning step, run a lightweight probe to verify access (see orchestrator.md — Try-First Rule for the probe table). If the probe succeeds, proceed. If it fails, log the exact error and escalate as a blocker. Example chain: create Neon DB → receive `DATABASE_URL` → `doppler secrets set` → `gh secret set` → `mcp__railway__set-variables`. Each step in the chain is independent — if one fails, only that step is a blocker.
- **Environment Variable Confirmation (mandatory after environment provisioning):** After completing environment provisioning, produce an "Environment Variable Confirmation" section in the devops setup artifact listing every platform env var (Vercel, Cloudflare, Railway, Neon) with its configured value and the expected value from the **Environment Variable Contract** (`env-contract.md`). The devops-engineer must use the exact key names from the contract — never rename, abbreviate, or invent env var names. Any env var configured on a platform whose key name does not exactly match the contract is a **Critical** finding. This confirmation is consumed by the compliance-officer and security-auditor before they issue their verdicts
...
## CI/CD Gating Policy
For each job in each workflow, document whether it is **blocking** (pipeline halts on failure) or **informational** (failure logged but pipeline continues) for each environment:
...
Items in "Outstanding Actions" must be surfaced to the deploy-environment agent as pre-deployment gate checks. Advisory notes ("should be disabled") are prohibited — every action item must be explicitly tracked as DONE or NOT DONE.
## PILOT STATUS
