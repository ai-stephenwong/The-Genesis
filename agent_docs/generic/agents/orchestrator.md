...
- **Pipeline execution log (append-only):** At the start of every pipeline run, copy `agent_docs/templates/pipeline-log.md` to `artifacts/runs/log-YYYYMMDD-HHmm.md` and fill in the run header (run ID, started, mode, Genesis version). Append an entry for every significant event: stage start/complete/fail, validation results, remediation routing, gate decisions, snapshots, escalations, and process observations. Never edit or delete existing entries — only append. This is the human-readable flight recorder; the retrospective-analyst reads it to understand what happened and why
