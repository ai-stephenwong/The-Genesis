...
- **Configure or document any deployment method that bypasses the CI/CD pipeline** — all staging and production deployments must be triggered by `git push` to the correct branch; GitHub Actions then runs `vercel deploy` / `wrangler deploy` inside CI. Running these commands directly from a developer machine or from Claude Code is not permitted for staging or production. The sole exception is the one-time first-deployment URL wiring (§4 of `deployment-vercel-cloudflare.md`). Deployment credentials must exist only inside the CI/CD system (GitHub Actions secrets), never distributed locally.
